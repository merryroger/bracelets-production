...
const Pager = class ObjectPager {
  ...
  constructor(settings = {}) {
    ...
    this._swipInfo = {
      ...
      update: (x) => {
        (this._swipInfo.dir =
          x == this._swipInfo.lastX.x ? 0 : x < this._swipInfo.lastX ? 1 : -1),
          (this._swipInfo.lastX = x);
      },
    };
    this._setUp(settings);
    this._nullPos =
      this._settings.frameDock.offsetWidth - this._settings.band.offsetWidth;
  ...
  _reCalcSizes() {
    ...
    const frameRect = (this._parameters.frameRect =
      this._settings.frameDock.getBoundingClientRect());
    const bandRect = (this._parameters.bandRect =
      this._settings.band.getBoundingClientRect());
    ...
    const gap = (rowMaxCapacity < 2)
        ? 0
        : (this._parameters.gap = Math.floor((bandRect.width - itemRect.width * rowMaxCapacity) / (rowMaxCapacity - 1)));
  ...
  _rebuildControls(cnt) {
    const controls = Array.from(
      this._settings.ctrlDock.querySelectorAll("label")
    );
    let ctrlSet = [];
  ...
  _hangTouchHandleListeners() {
    document.addEventListener("touchstart", (e) => {
      if (
        this._parameters.movesNeeded > 1 &&
  ...
  _hangMouseHandleListeners() {
    this._settings.band.addEventListener(
      "mousedown",
      (e) => {
        if (
          this._parameters.movesNeeded > 1 &&
  ...
  _getCardAtRightPosition(cv) {
    return cv == 1
      ? 1
      : cv == this._parameters.movesNeeded
      ? this._settings.items.length
  ...
  _reCalcAfterResize() {
    if (this._parameters.ctrlDockWidth != this._settings.ctrlDock.offsetWidth) {
      ...
      let atRightEdge = this._getCardAtRightPosition(cv);
      const focusedCard = this._getFocusedCardNum();
      this._parameters.shift = +getComputedStyle(this._settings.band).transform.split(", ")[4];

      this._reCalcSizes();

      atRightEdge = (focusedCard && (focusedCard - atRightEdge) >= this._parameters.itemsPerScreen) ? focusedCard : atRightEdge;

      cv = Math.floor(
        (atRightEdge - 1) / (this._parameters.itemsPerScreen * this._parameters.rows)
      );
  ...
  _pointerUp(e) {
    const dX = e.clientX - this._swiX;
    this._settings.band.style.setProperty("transform", `translateX(${dX}px)`);
    this._reCalcAfterSwip(dX - this._nullPos);
  }
